...
class TerminalLauncher:
    ...
    def check_venv(self) -> bool:
        ...
        if not activate_script.exists():
            self.logger.error(f"激活脚本不存在: {activate_script}")
            return False
    ...
    def _write_pids(self) -> None:
        ...
        try:
            self.logs_path.mkdir(exist_ok=True)
            backend_pid = self.backend_process.pid if self.backend_process and self.backend_process.poll() is None else ""
            ui_pid = self.ui_process.pid if self.ui_process and self.ui_process.poll() is None else ""
            with open(self.pids_file, "w", encoding="utf-8") as f:
                f.write(f"backend_pid={backend_pid}\n")
                f.write(f"ui_pid={ui_pid}\n")
        except Exception as e:
            self.logger.debug(f"写入PID文件失败: {e}")

    def start_backend(self) -> bool:
        """启动后端服务"""
        try:
            if self.backend_process and self.backend_process.poll() is None:
                self.logger.info(f"后端已在运行（PID={self.backend_process.pid}），跳过重复启动")
                self._write_pids()
                return True
            ...
            if not backend_main.exists():
                self.logger.error(f"后端主文件不存在: {backend_main}")
                return False
            ...
            self.logger.info(f"后端启动成功，PID={self.backend_process.pid}")
            self._write_pids()
            return True
        except Exception as e:
            self.logger.error(f"启动后端失败: {e}")
            return False
    ...
    def start_ui(self) -> bool:
        ...
        try:
            if self.ui_process and self.ui_process.poll() is None:
                self.logger.info(f"UI已在运行（PID={self.ui_process.pid}），跳过重复启动")
                self._write_pids()
                return True
            ...
            if not ui_main.exists():
                self.logger.error(f"UI主文件不存在: {ui_main}")
                return False
            ...
            self.logger.info(f"UI启动成功，PID={self.ui_process.pid}")
            self._write_pids()
            return True
        except Exception as e:
    ...
    def stop_all(self) -> None:
        ...
        if self.backend_process:
            try:
                ...
                self.logger.error(f"停止后端进程失败: {e}")
            finally:
        ...
        if self.ui_process:
            try:
                ...
                self.logger.error(f"停止UI进程失败: {e}")
            finally:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "ui_pid": self.ui_process.pid if self.ui_process and self.ui_process.poll() is None else None,
            "watchdog_running": self.watchdog_thread is not None and self.watchdog_thread.is_alive(),
    ...
    def run_diagnostics(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "backend_main_exists": (self.backend_path / "core" / "vnpy_integration.py").exists(),
            "ui_main_exists": (self.ui_path / "main_window.py").exists(),
            "logs_writable": os.access(str(self.logs_path), os.W_OK) if self.logs_path.exists() else True,
        }
        return info
        ...
        while True:
            try:
                ...
                if self.backend_process and self.backend_process.poll() is not None:
                    ...
                    if exit_code != 0:
                        self.logger.warning(f"检测到后端异常退出(code={exit_code})，准备重启")
                ...
                if self.ui_process and self.ui_process.poll() is not None:
                    ...
                    if exit_code != 0:
                        self.logger.warning(f"检测到UI异常退出(code={exit_code})，准备重启")
                        if self.auto_restart:
                            time.sleep(self.restart_delay)
                            self.start_ui()
                    else:
            ...
            try:
                if self.cmd_file.exists():
                    ...
                    try:
                        ...
                        content = ""
                    if content:
                        self.logger.info(f"收到指令: {content}")
                        # 清空指令文件避免重复
                        try:
    ...
    def start_watchdog(self) -> None:
        ...
        if not (self.cmd_listener_thread and self.cmd_listener_thread.is_alive()):
            self.cmd_listener_thread = threading.Thread(target=self._command_listener_loop, daemon=True)
...
def main() -> int:
    ...
    print("📊 当前状态：")
    print(f"  - 后端运行: {'✅' if status['backend_running'] else '❌'} (PID={status['backend_pid']})")
    print(f"  - UI运行: {'✅' if status['ui_running'] else '❌'} (PID={status['ui_pid']})")
    print(f"  - 热更新: {'✅' if status['hot_reload_enabled'] else '❌'}")

    print("\n💡 提示：Ctrl+C 退出；变更代码后可向 logs/launcher.cmd 写入 restart_ui 或 restart_backend 测试热更新")
