...
class OpsCenter(QWidget):
    ...
    def _connect_signals(self):
        ...
        self.restart_backend_btn.clicked.connect(lambda: self._write_cmd("restart_backend"))
        self.restart_all_btn.clicked.connect(lambda: self._write_cmd("restart_all"))
        self.toggle_hot_reload_btn.clicked.connect(lambda: self._write_cmd("restart_all"))
    ...
    def _refresh_status(self):
        ...
        self.backend_status.setText(f"后端: {'运行中' if backend_alive else '未运行'} ({pids.get('backend_pid')})")
        self.ui_status.setText(f"UI: {'运行中' if ui_alive else '未运行'} ({pids.get('ui_pid')})")
