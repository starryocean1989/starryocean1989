...
class ThemeManager:
    ...
    def __init__(self, theme_path: Optional[str] = None):
        ...
        self.theme_path = theme_path or str(Path(__file__).parent / "dark_theme.json")
    ...
    def _load_themes(self):
        ...
        try:
            ...
            self._logger.info(f"成功加载主题: {self.theme_path}")
        except Exception as e:
            self._logger.error(f"加载主题失败: {e}")
    ...
    def _set_dark_palette(self, app: QApplication, theme: Dict[str, Any]):
        ...
        # 交替基础颜色
        alternate_bg = background.get("secondary", "#1e1e1e") if isinstance(background, dict) else "#1e1e1e"
        ...
        tooltip_bg = tooltip.get("background", "#383838") if isinstance(tooltip, dict) else "#383838"
        ...
        dark_palette.setColor(QPalette.Disabled, QPalette.Button, QColor(disabled_bg))

        disabled_text = text.get("disabled", "#808080") if isinstance(text, dict) else "#808080"
        dark_palette.setColor(QPalette.Disabled, QPalette.ButtonText, QColor(disabled_text))
    ...
    def _apply_global_stylesheet(self, app: QApplication, colors: Dict[str, Any]):
        ...
        QComboBox QAbstractItemView {{
            background-color: {safe_get_color('surface.main', '#1e1e1e')};
            color: {safe_get_color('text.primary', '#ffffff')};
            selection-background-color: {safe_get_color('primary', '#1e88e5')};
        }}

        QScrollBar:vertical {{
            background-color: {safe_get_color('scrollbar.background', '#2d2d2d')};
            width: 12px;
            margin: 0px;
        }}
