...
class PortfolioService(BaseService):
    ...
    def _calculate_performance_curve(self, trades: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 尝试使用原生扩展（优先）
            if self._use_native_finance_ops and native_aggregate_daily_pnl is not None:
                try:
                    # 准备数据：提取日期和pnl
                    dates = []
                    pnls = []
                    ...
                    if dates:
                        # 调用原生算子
                        result = native_aggregate_daily_pnl(dates, pnls)
                        ...
                        for i in range(len(result['dates'])):
                            date_int = int(result['dates'][i])
                            date_str = f"{date_int//10000:04d}-{(date_int//100)%100:02d}-{date_int%100:02d}"
                            daily_pnl = result['daily_returns'][i]
                            cumulative_pnl = result['cumulative_equity'][i] - 1.0  # 减去初始值1.0
                            equity = initial_capital * result['cumulative_equity'][i]
                            return_rate = cumulative_pnl
                            
                            performance_curve.append({
                                "date": date_str,
                                "daily_pnl": round(daily_pnl, 2),
                                "cumulative_pnl": round(cumulative_pnl * initial_capital, 2),
                                "equity": round(equity, 2),
                                "return_rate": round(return_rate * 100, 2),  # 百分比
                            })
                        
                        self.logger.debug("✅ 使用原生算子计算业绩曲线", extra={"log_type": "PROGRESS"})
                        return performance_curve
                        
                except Exception as e:
                    # 原生扩展失败，记录警告并降级到Python实现
                    self.logger.warning(
                        f"原生算子失败，降级到Python实现: {e}",
                        extra={"log_type": "ALERT"},
                        exc_info=True
                    )
