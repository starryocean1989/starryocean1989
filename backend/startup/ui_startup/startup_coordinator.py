    ...
    def run(self):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                self.logger.info("[BACKEND-INIT] 开始初始化数据服务...")
                try:
                    ...
                    try:
                        ...
                        if init_success:
                            ...
                            self.logger.warning("[BACKEND-INIT] ⚠️ ChinaStockEngine 初始化失败")
                    except Exception as e:
                        self.logger.error(f"[BACKEND-INIT] ❌ ChinaStockEngine 初始化异常: {e}")
                        init_success = False
                    ...
                    try:
                        ...
                        if data_init_success:
                            ...
                            self.logger.warning("[BACKEND-INIT] ⚠️ DataCenterService 初始化失败")
                    except Exception as e:
                        self.logger.error(f"[BACKEND-INIT] ❌ DataCenterService 初始化异常: {e}")
                        data_init_success = False
                    ...
                    if service_success:
                        ...
                        self.logger.warning("[BACKEND-INIT] ⚠️ 数据服务初始化部分失败")
