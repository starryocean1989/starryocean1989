    ...
    async def _execute(self, context: StartupContext) -> StageResult:
        ...
        try:
            ...
            if context.backend_initialized:
                # 后端已就绪，初始化功能界面
                stage_logger.info("📍 阶段4.1: 初始化功能界面", extra={"log_type": "STAGE_NODE"})
                main_window.initialize_function_interfaces_after_backend()
                stage_logger.info("✅ 功能界面初始化完成", extra={"log_type": "STAGE_NODE"})
            else:
                # 后端未就绪（理论上不应该发生，因为阶段4在阶段3之后）
                stage_logger.warning("⚠️ 后端未就绪，功能界面将延迟初始化", extra={"log_type": "STAGE_NODE"})

            # 关闭启动画面（如果有的话）
            # 注意：实际启动画面可能在MainWindow创建时自动关闭
            stage_logger.info("✅ 启动画面关闭", extra={"log_type": "STAGE_NODE"})
            ...
            stage_logger.info(f"✅ UI就绪 ({elapsed_ms:.0f}ms)", extra={"log_type": "STAGE_NODE"})
