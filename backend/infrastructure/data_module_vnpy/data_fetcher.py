...
class StockFetcher:
    ...
    def _download_tasks_for_server(
        ...
        try:
            ...
            self.logger.info("服务器 %s 开始处理 %d 个任务", server_info, len(tasks))
            ...
            for idx, (symbol, interval) in enumerate(tasks):
                ...
                try:
                    ...
                    self.logger.error(
                        "服务器 %s 下载 %s %s 失败: %s", server_info, symbol, interval, e
                    )

            self.logger.info(
                "服务器 %s 完成: 成功 %d/%d 个任务", server_info, len(results), len(tasks)
            )
