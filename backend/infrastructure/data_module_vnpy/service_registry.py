    ...
    def register_service(self, service: VnPyService) -> bool:
        ...
        try:
            if service.name in self.services:
                logger.warning("服务 %s 已存在,将被覆盖", service.name)

            self.services[service.name] = service
            logger.info("服务 %s 注册成功", service.name)
            return True

        except Exception as e:
            logger.error("注册服务 %s 失败: %s", service.name, e)
            return False
    ...
    def unregister_service(self, name: str) -> bool:
        ...
        try:
            if name in self.services:
                ...
                logger.info("服务 %s 注销成功", name)
                return True
            else:
                logger.warning("服务 %s 不存在", name)
                return False

        except Exception as e:
            logger.error("注销服务 %s 失败: %s", name, e)
            return False
        ...
        try:
            for name, service in self.services.items():
                try:
                    ...
                    logger.error("关闭服务 %s 失败: %s", name, e)

            self._initialized = False
            logger.info("所有VnPy服务已关闭")

        except Exception as e:
            logger.error("关闭VnPy服务失败: %s", e)
