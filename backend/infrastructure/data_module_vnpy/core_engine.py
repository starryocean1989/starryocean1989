    ...
    def load_with_validation(cache_file: Path) -> Tuple[Any, str, bool]:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"[DailyCacheManager] 文件元信息获取完成: 文件路径={cache_file_path}, "
                    f"修改时间={file_mtime.isoformat()}, 文件大小={file_size}字节, 耗时={file_stat_elapsed:.1f}ms",
                    extra={"log_type": "SYSTEM"}
                )
            except Exception as stat_e:
                logger.debug(
                    f"[DailyCacheManager] 文件元信息获取失败: 文件路径={cache_file_path}, 错误={stat_e}",
                    extra={"log_type": "SYSTEM"}
                )
                file_mtime = None
                file_size = 0
            ...
            logger.debug(
                f"[DailyCacheManager] 文件读取完成: 文件路径={cache_file_path}, "
                f"JSON解析耗时={file_read_elapsed:.1f}ms",
                extra={"log_type": "SYSTEM"}
            )
            ...
            if not cache_date_str:
                logger.warning(
                    f"⚠️ 缓存缺少日期字段: 文件路径={cache_file_path}",
                    extra={"log_type": "SYSTEM"}
                )
                return data, "", False
            ...
            if is_valid:
                logger.debug(
                    f"[DailyCacheManager] 缓存有效: 文件路径={cache_file_path}, 日期={cache_date}",
                    extra={"log_type": "SYSTEM"}
                )
            else:
                # 🔧 修复：详细记录日期不匹配的原因
                logger.debug(
                    f"[DailyCacheManager] 缓存已过期: 文件路径={cache_file_path}, "
                    f"缓存日期={cache_date}, 真实日期={real_date}, 系统日期={system_date}",
                    extra={"log_type": "SYSTEM"}
                )
            ...
            logger.debug(
                f"[DailyCacheManager] load_with_validation完成: 文件路径={cache_file_path}, "
...
class ChinaStockEngine:
    ...
    def _smart_cache_validation_and_sensing(
        ...
        try:
            ...
            stage_logger.info("┌─ 步骤1: 服务器池验证与测速 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            if step1_result.get("offline_mode"):
                ...
                stage_logger.critical("│ 🔥 所有TDX服务器不可用，系统无法获取数据", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                stage_logger.warning("│ ⚠️ 进入离线降级模式", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                stage_logger.info("└─────────────────────────────┘", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                stage_logger.info("⏭️ 跳过步骤2-7，直接执行步骤8", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                # 设置离线模式
                ...
                stage_logger.info("│ ✅ LoadBalancer初始化完成", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                stage_logger.info("│   - 任务分类体系: NETWORK_DOWNLOAD, LOCAL_SCAN, LOCAL_READ", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                stage_logger.info("│   - 队列压力监控: 正常/高/临界三级", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                stage_logger.info("└─────────────────────────────┘", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            stage_logger.info(f"✅ 步骤1完成 ({step1_time:.0f}ms) [进度: 12%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            if not goto_step_8:
                ...
                stage_logger.info("┌─ 步骤2: 获取当前日期（网络时间）─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info(f"✅ 步骤2完成 ({step2_time:.0f}ms) [进度: 25%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info("┌─ 步骤3: 验证交易日历缓存 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info(f"✅ 步骤3完成 ({step3_time:.0f}ms) [进度: 37%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info("┌─ 步骤4: 验证品种列表缓存 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                if not step4_result.get("cache_valid") or step4_result.get("cache_missing"):
                    ...
                    if self.symbol_loader is None:
                        ...
                        self.symbol_loader = SymbolLoader(self.event_engine)
                    # 使用共享连接池重新加载（传递共享连接池）
                    self.symbol_loader.reload_and_classify(shared_retry_pool=self._shared_retry_pool)
                ...
                stage_logger.info(f"✅ 步骤4完成 ({step4_time:.0f}ms) [进度: 50%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info("┌─ 步骤5: 验证IPO日期缓存 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info(f"✅ 步骤5完成 ({step5_time:.0f}ms) [进度: 62%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info("┌─ 步骤6: 更新本地数据索引 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info(f"✅ 步骤6完成 ({step6_time:.0f}ms) [进度: 75%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info("┌─ 步骤7: 检查数据更新状态 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
                ...
                stage_logger.info(f"✅ 步骤7完成 ({step7_time:.0f}ms) [进度: 87%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            stage_logger.info("┌─ 步骤8: 启动文件监控 ─┐", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            stage_logger.info(f"✅ 步骤8完成 ({step8_time:.0f}ms) [进度: 100%]", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            stage_logger.info(f"✅ 缓存验证与感知流程完成 (总耗时: {elapsed_time:.1f}s)", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            stage_logger.info("✅ 数据引擎完全就绪", extra={"log_type": "STAGE_NODE", "scenario": scenario})
            ...
            if self._shared_retry_pool is None:
                try:
                    ...
                    self._shared_retry_pool = pool_manager.get_or_create_retry_pool(
                        keep_pool=True,
                        max_connections=None  # None表示自动计算，每个服务器一个连接
                    )
        ...
        try:
            ...
            if not is_valid:
                ...
                if self._shared_retry_pool:
                    try:
                        # 获取连接池统计信息
                        if hasattr(self._shared_retry_pool, '_phase1_pool') and self._shared_retry_pool._phase1_pool:
                        ...
                        logger.debug(f"[步骤4-性能埋点] 获取连接池状态失败: {e}", extra={"log_type": "SYSTEM"})
                ...
                throughput = (total_symbols / (reload_classify_elapsed / 1000)) if reload_classify_elapsed > 0 else 0
    ...
    def _validate_ipo_cache(self, current_date, stage_logger) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_valid and data and isinstance(data, dict):
                ...
                pool_manager = get_server_pool_manager()
                max_concurrent_limit = pool_manager.calculate_total_max_connections()  # 直接累加所有服务器的max_connections
                ...
                success_count = sum(1 for v in ipo_dates.values() if v is not None) if ipo_dates else 0
                throughput = (total_tasks / (download_elapsed / 1000)) if download_elapsed > 0 else 0
    ...
    def _update_local_data_index(self, stage_logger) -> Dict[str, Any]:
        ...
        try:
            # 🔍 详细埋点：步骤开始
            logger.debug(
                f"[步骤6] _update_local_data_index开始",
                extra={"log_type": "SYSTEM"}
            )
    ...
    def _check_data_update_status(self, stage_logger) -> Dict[str, Any]:
        ...
        try:
            # 🔍 详细埋点：步骤开始
            logger.debug(
                f"[步骤7] _check_data_update_status开始",
                extra={"log_type": "SYSTEM"}
            )
            ...
            if self.unified_data_manager is None:
                ...
                unifieddatamanager_init_elapsed = (time.time() - unifieddatamanager_init_start_time) * 1000
                logger.debug(
                ...
                logger.debug(
                    f"[步骤7] UnifiedDataManager已存在，跳过初始化",
                    extra={"log_type": "SYSTEM"}
                )
