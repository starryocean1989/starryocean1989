...
LoadBalancer是一个统一的智能负载均衡系统，用于动态调整系统中各类任务的并发配置，以应对不同的系统资源压力。本模块集成了服务器池管理器、两段式下载机制、持久化进程池、自适应批次计算、共享内存架构、增量扫描、流式处理、GPU加速、性能监控、动态进程池、连接生命周期管理等完整特性。

**极限合并**: 已将原10个独立文件（含v3.7新增的dynamic_process_pool.py）合并为1个统一文件`load_balancer.py`（7,450+行），大幅提升调试体验和代码可维护性。

**v3.7新特性**: 集成`DynamicProcessPool`和`ConnectionLifecycleManager`，提供标准化的连接创建、健康检查、清理流程，彻底解决`socket.send() raised exception`问题。

### 核心特性

- ✅ **统一连接生命周期管理**（v3.7新增）：ConnectionLifecycleManager提供标准化连接管理，解决资源泄漏 ⭐ 最新
- ✅ **动态进程池管理**（v3.7集成）：DynamicProcessPool支持运行时动态增减进程 ⭐ 最新
- ✅ **event_loop_lag指导的并发决策**（v3.3新增）：基于协程排队指标智能调整并发
- ✅ **纯协程+进程架构**（v3.3优化）：移除隐式线程池，统一使用多进程+多协程
...
```
LoadBalancer 模块（极限合并：10个文件 → 1个文件）
└── load_balancer.py (7,450行) - 统一负载均衡模块
    │
    ├── 第1部分：任务定义和基础配置（~600行）
    ...
    │
    └── 第11部分：进程池与连接管理（~270行，v3.7新增）
        ├── DynamicProcessPool            # 动态进程池管理器
        └── ConnectionLifecycleManager    # 连接生命周期管理器
```

### 极限合并成果

**合并效果**：10个Python文件 → 1个Python文件（减少90%）
