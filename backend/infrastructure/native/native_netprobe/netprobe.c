...
static int
ensure_connectex(int family, LPFN_CONNECTEX *out_fn)
{
    ...
    SOCKET temp = WSASocket(family, SOCK_STREAM, IPPROTO_TCP, NULL, 0, WSA_FLAG_OVERLAPPED);
...
static void
cleanup_request(ProbeRequest *req)
{
...
static int
prepare_request(ProbeRequest *req, const char *host_utf8, unsigned short port)
{
    ...
    int gai = getaddrinfo(host_utf8, port_buf, &hints, &result);
    if (gai != 0 || !result) {
        req->status = -1;
...
static int
bind_local_address(SOCKET sock, int family)
{
    if (family == AF_INET) {
...
static int
start_request(ProbeRequest *req, HANDLE iocp)
{
    ...
    SOCKET sock = WSASocket(req->family, SOCK_STREAM, IPPROTO_TCP, NULL, 0, WSA_FLAG_OVERLAPPED);
...
static void
finalize_request(ProbeRequest *req, int status, int error_code, double now)
{
    if (req->completed) {
        return;
    }
...
static PyObject *
build_result(const ProbeRequest *req)
{
    ...
    if (req->status == 0) {
        ...
        PyDict_SetItemString(result, "ping_ms", PyFloat_FromDouble(req->latency_ms >= 0.0 ? req->latency_ms : 0.0));
...
static PyObject *
run_batch(PyObject *server_list, double timeout, int max_concurrent)
{
    ...
    for (Py_ssize_t i = 0; i < total; ++i) {
        ...
        if (prepare_request(&requests[i], host_utf8, (unsigned short)port_long) == 0) {
            prepared++;
        }
    }
    ...
    while (completed < total) {
        ...
        DWORD wait_ms = (DWORD)(timeout_ms > 0.0 ? timeout_ms : 1.0);
        ULONG_PTR key = 0;
        LPOVERLAPPED overlapped = NULL;
        DWORD bytes = 0;
        BOOL ok = GetQueuedCompletionStatus(iocp, &bytes, &key, &overlapped, wait_ms);
        double now = now_ms();
    ...
    PyDict_SetItemString(summary, "duration_ms", PyFloat_FromDouble(now_ms() - start_batch));

    return summary;
}

static PyObject *
py_batch_test_connections(PyObject *Py_UNUSED(self), PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|di", kwlist, &servers, &timeout, &max_concurrent)) {
        return NULL;
    }
...
static PyObject *
py_test_connection(PyObject *Py_UNUSED(self), PyObject *args, PyObject *kwargs)
{
...
static PyMethodDef module_methods[] = {
    {"test_connection", (PyCFunction)py_test_connection, METH_VARARGS | METH_KEYWORDS, PyDoc_STR("test_connection(host: str, port: int, timeout=3.0) -> dict")},
    {"batch_test_connections", (PyCFunction)py_batch_test_connections, METH_VARARGS | METH_KEYWORDS, PyDoc_STR("batch_test_connections(servers, timeout=3.0, max_concurrent=32) -> dict")},
    {NULL, NULL, 0, NULL},
};
