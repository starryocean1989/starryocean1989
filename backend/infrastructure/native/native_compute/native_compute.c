...
static PyMethodDef ComputeMethods[] = {
    {"batch_compute", batch_compute_func, METH_VARARGS,
     "Batch numerical operations"},
    {"batch_hash", batch_hash_func, METH_VARARGS, "Batch hash computation"},
    {"batch_get_price", batch_get_price_func, METH_VARARGS,
     "Batch get_price parsing for TDX"},
    {"batch_validate_iso_dates", batch_validate_iso_dates, METH_VARARGS,
     "Batch validate ISO date strings"},
    {"batch_compare_dates", batch_compare_dates, METH_VARARGS,
     "Batch compare dates with reference"},
    {"prefix_sum_scale", prefix_sum_scale_func, METH_VARARGS,
     "Prefix sum with scaling for price diffs"},
    {NULL, NULL, 0, NULL}};
...
PyMODINIT_FUNC PyInit_native_compute(void) {
  PyObject *m = PyModule_Create(&native_computemodule);
  if (m) {
    NATIVE_LOG_INFO("backend.native.compute.module", "PyInit_native_compute", __LINE__,
                    "native_compute module loaded");
  } else {
    NATIVE_LOG_ERROR("backend.native.compute.module", "PyInit_native_compute", __LINE__,
                     "failed to create native_compute module");
  }
  return m;
}
