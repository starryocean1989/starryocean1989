# -*- coding: utf-8 -*-
"""
TdxTradeServer Debug Version Build Script.

This script builds a debug version of TdxTradeServer by packaging
dependencies and executable files into a distributable archive.
"""
...
def get_version(base_path):
    """Get version from ChangeLog.md file."""
    with open(base_path + "\\ChangeLog.md", "rb") as f:
        version = f.readline().split(b" ")[0]
        build_ver = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        full_ver = version.decode("utf-8") + "_" + build_ver
        return full_ver
...
if __name__ == "__main__":
    ...
    deps_path = os.path.join(bpath, "debug_version_dependencies")
    for filename in os.listdir(deps_path):
        shutil.copy(
    ...
    exec_path = (
        os.path.join(
            os.path.dirname(bpath),
            "build-TdxTradeServer-Desktop_Qt_" + QT_VERSION +
            "_MSVC2015_32bit-Debug",
        )
        + "\\debug\\TdxTradeServer.exe"
    )
    ...
    if os.path.exists(exec_path):
        ...
        os.chdir(out_path)
        shutil.make_archive(
            archive_name, "zip", root_dir=files_path, base_dir="."
        )
        p("Done!")
    else:
