...
# 🚀 原生IOCP异步文件I/O：优先使用Windows IOCP，自动降级到executor
try:
...
async def _async_write_text(filepath: Path, content: str) -> None:
    ...
    try:
        if _USE_IOCP and compat_aopen is not None:
            ...
            async with file_obj:
                await file_obj.write(content_bytes)
        else:
            # 降级到executor
            def _write():
        ...
        logger.warning(f"异步写入文本文件失败，降级到executor: {e}", extra={"log_type": "SYSTEM"})
        # 最终降级到executor
        def _write():
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
