...
async def apply_adjustment(
    ...
    try:
        ...
        if xdxr_data is None or xdxr_data.empty:
            logger.warning(
                f"股票{symbol}无除权除息信息，返回原始数据", extra={"log_type": "SYSTEM"}
            )
            return df
...
async def _get_xdxr_info_async(
    symbol: str, market: int, client: Optional[AsyncTdxHq_API] = None
) -> pd.DataFrame:
    """
    异步获取除权除息信息
    ...
    try:
        # 如果未提供客户端，临时创建一个
        if client is None:
            from ..network.constants import HQ_HOSTS

            # 使用第一个可用服务器临时获取数据
            for server in HQ_HOSTS[:5]:  # 尝试前5个
                temp_client = await AsyncTdxHq_API.factory(
                    server=(server[1], server[2]), timeout=3.0, raise_exception=False
                )
                if temp_client:
                    client = temp_client
                    need_close = True
...
def _apply_full_adjustment(
    bfq_data: pd.DataFrame, xdxr_data: pd.DataFrame, adjust_type: str
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        data = pd.concat(
            [
                data,
                info.loc[
                    data.index[0] : data.index[-1], ["fenhong", "peigu", "peigujia", "songzhuangu"]
                ],
            ],
            axis=1,
        )
        ...
        data = pd.concat(
            [data, info.loc[:, ["category", "fenhong", "peigu", "peigujia", "songzhuangu"]]], axis=1
        )
    ...
    try:
        async with httpx.AsyncClient(verify=False, timeout=5.0) as client:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"获取新浪复权因子失败: HTTP {response.status_code}",
